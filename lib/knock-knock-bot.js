    			...
    			case 'monitors':
    				...
    				self.db.each(query, params, function(err, row) {
    						...
    						if (err) {
    							...
    							if (parts[1] == 'all') {
    								monitorString += ' (Channel: ' + self._getNameById(row.channel).name + ')';
    							}
...
KnockKnockBot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.channels[0].name, 'KnockKnockBot ready!', {as_user: true});
};
...
KnockKnockBot.prototype._getChannelById = function (channelId) {
    return this.channels.filter(function (item) {
        return item.id === channelId;
    })[0];
};

KnockKnockBot.prototype._getUserById = function (userId) {
    return this.users.filter(function (item) {
        return item.id === userId;
    })[0];
};

KnockKnockBot.prototype._getGroupById = function (groupId) {
    return this.groups.filter(function (item) {
        return item.id === groupId;
    })[0];
};

KnockKnockBot.prototype._getNameById = function(id) {
	if (id[0] === 'C' || id[0] === 'D') {
		return this._getChannelById(id);
	} else if (id[0] === 'G') {
